...
- **A fix restores intent; an addition changes it — and only a founder decides an addition** (`constitution.md` → The Workflow, "A fix restores intent"). The passed-by rule above sends you *looking* for defects; it is no license to *build*. Before you act on anything you noticed, ask one question — does this change make the code do what it was **already designed** to do (a wrong type, a guard the design assumed, a stale doc, a broken wire), or make the system do something it **never did** (a check that never existed, a resource nothing declared, a design choice reopened)? The first is a **fix**, and the two paths above apply. The second is an **addition**, whatever its size — new lines where none existed are only the *tell*; **intent is the test**. An addition is never begun: no branch, no spike, no "small version." It is **filed as an issue** stating **what it adds, how you would build it, and why you believe it is needed**, carrying **both `needs-human` and `Do Not Work`**, and then **nothing further happens until a founder decides** — agents propose; founders decide what the system becomes. **When you cannot tell which it is, it is an addition.**
