...
- **`handoff` label present → router-wired (on-server) → LABEL, do NOT paste.** File the issue, then **apply the `handoff` label** (as a trusted actor — Drawk or any fleet bot). GitHub fires `issues.labeled` → the App webhook → the router on the fleet server, which drops to the agent's OS user and launches it with a trigger *the router itself synthesizes* (`Cross-repo handoff: work <issue-url>`, plus an input-security preamble). **Creating the issue alone does nothing — the label is the trigger.** Never hand Drawk a paste prompt for these repos; there is no human in the loop.
